...
class ParaDDIMScheduler(DDIMScheduler):
    ...
    def batch_step_with_noise(
        ...
        if eta > 0:
            ...
            if variance_noise is None:
                variance_noise = randn_tensor(
                    model_output.shape, generator=generators, device=model_output.device, dtype=model_output.dtype
                )
    ...
    def set_coarse_and_fine_timesteps(self, num_time_subintervals: int,num_preconditioning_steps: int):
        ...
        for i,(start_idx,end_idx) in enumerate(interval_indexs):
            ...
            # sigmas = self.sigmas[start_idx:end_idx]
            # if interval_len[i] < max_interval_len:
            ...
            # sigmas_matrix.append(torch.cat([sigmas,self.sigmas[-1].unsqueeze(0)]))
    ...
    def ddim_step(
        ...
        Args:
            ...
            eta (`float`):
                The weight of noise for added noise in diffusion step.
            use_clipped_model_output (`bool`, defaults to `False`):
                If `True`, computes "corrected" `pred_original_sample` from the clipped predicted original sample. Necessary
                because predicted original sample is clipped to [-1, 1] when `self.config.clip_sample` is `True`. If no
                clipping has happened, "corrected" `pred_original_sample` would coincide with the one provided as input and
                `use_clipped_model_output` has no effect.
            generator (`torch.Generator`, *optional*):
                A random number generator.
            variance_noise (`torch.Tensor`):
        ...
        if eta > 0:
            ...
            if variance_noise is None:
                variance_noise = randn_tensor(
                    pred_original_sample.shape, generator=generator, device=pred_original_sample.device, dtype=pred_original_sample.dtype
                )
            variance = std_dev_t * variance_noise
