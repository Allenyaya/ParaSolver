...
class ParaSolverDPMSolverStableDiffusionPipeline(StableDiffusionPipeline):
    def __init__(
        self,
        vae: AutoencoderKL,
    ...
    def preparation_for_diffusion(
        ...
        **kwargs,
        ...
        # print("def preparation_for_diffusion: timesteps, num_inference_steps",timesteps, num_inference_steps)
    ...
    def parasolver_forward(
        ...
        for j in range(num_time_subintervals):
            ...
            noise = (self.scheduler._get_variance(cur_t)**2) * variance_noise
                ...
                for i, t in enumerate(timesteps):
                    if num_preconditioning_steps>0:
                        if i < num_preconditioning_steps:
                            ...
                            latents, _ = self.scheduler.dpmsolver_step(
    ...
    def initialize_points_outside_current_window(
        ...
        """Updates latent representations for non-convergence timesteps using DPMSolver steps.

        This function performs DPMSolver sampling steps between specified start and end points
        ...
        Args:
            ...
            eta (float, optional): DPMSolver noise parameter (0=deterministic). Defaults to 0.0.
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    if i >= start_point and i < end_point:
                        ...
                        image, _ = self.scheduler.dpmsolver_step(
                            X_0, t, image,
                            generator=generator, return_dict=False
                        )
                        progress_bar.update()
