...
class ParaSolverDDPMStableDiffusionPipeline(StableDiffusionPipeline):
    def __init__(
        self,
        vae: AutoencoderKL,
        ...
        self.parallel_resulted_latents  = dict()
    ...
    def preparation_for_diffusion(
        ...
        **kwargs,
        ...
        self.parallel_resulted_latents[timesteps[0].item()]  = latents
    ...
    def parasolver_forward(
        ...
        self.preparation_for_diffusion(prompt=prompt,generator=generator,timesteps=self.scheduler.coarse_timesteps)
        ...
        #5.Prepare stop parameters
        inverse_variance_norm = 1. / torch.tensor(
            [self.scheduler._get_variance_v1(timesteps[j],timesteps[j + 1])
             for j in list(range(len(timesteps)-1)) + 2*[len(timesteps) - 2]]).to(torch.float32).to(device)
        ...
        #6.Set fine scheduler  for parallel iteration, such that  the scheduler can accurately find the previous timesteps or sigmas
        ...
        while i < num_time_subintervals:
            ...
            latents_time_evolution_buffer[i + 1:end_i + 1] = block_latents_new.clone()
    ...
    def preconditioning_initialization(
        ...
        """
        Performs the standard diffusion process with optional preconditioning steps,
        approximating epsilon (noise) values.

        Args:
            num_inference_steps: Total number of diffusion inference steps.
            timesteps: Tensor containing the timesteps for the diffusion process.
            image: Input image tensor to be processed.
            generator: Optional random number generator or list of generators for reproducibility.
            eta: Eta parameter controlling the stochasticity in DDIM sampling.
            use_clipped_model_output: Whether to clip the model output (optional).
            num_preconditioning_steps (int, optional): Number of preconditioning steps to run with full model guidance. Defaults to 0.

        Returns:
            Tuple containing:
            - stats_pass_count: Number of model passes performed.
            - stats_flop_count: Estimated floating point operations count.
            - initial_para_dur: Initial parallel duration time in milliseconds.
        Note:
                ...
                for i, t in enumerate(timesteps):
                    if num_preconditioning_steps>0:
                        if i < num_preconditioning_steps:
                            ...
                            generator = generator.manual_seed(t.item())
                            latents, _ = self.scheduler.ddpm_step(
                                self.predicted_original_sample, t, latents,
                                generator=generator, return_dict=True
                            ).values()
                    else:
    ...
    def initialize_points_outside_current_window(
        ...
        """Updates latent representations for non-convergence timesteps using DDPM steps.

        This function performs DDPM sampling steps between specified start and end points
